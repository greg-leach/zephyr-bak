 ...
 *
 * @param dev the MCP7940N device pointer.
 * @param time pointer to time_t value that will contain unix time if sucessful.
 *
 * @retval return 0 on success, or a negative error code from an I2C
 * transaction.
 */
 ...
 *
 * @param dev the MCP7940N device pointer.
 * @param time pointer to unix time that will be used to work out the weekday
 *
 * @retval return 0 on success, or a negative error code from an I2C
 * transaction or invalid parameter.
 */
static int set_day_of_week(const struct device *dev, time_t *time)
{
	struct mcp7940n_data *data = dev->data;
