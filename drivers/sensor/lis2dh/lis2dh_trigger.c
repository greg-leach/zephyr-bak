...
static int lis2dh_start_trigger_int1(struct device *dev)
{
	...
	status = gpio_pin_enable_callback(lis2dh->gpio_int1,
					  DT_LIS2DH_INT1_GPIO_PIN);
	if (unlikely(status < 0)) {
		LOG_ERR("enable callback failed err=%d", status);
	}

	return lis2dh_reg_write_byte(dev, LIS2DH_REG_INT1_CFG,
				     LIS2DH_ANYM_CFG);
}
				   ...
				   sensor_trigger_handler_t handler)
{
	...
	status = lis2dh_reg_write_byte(dev, LIS2DH_REG_INT1_CFG, 0);

	/* make sure any pending interrupt is cleared */
	status = lis2dh_reg_read_byte(dev, LIS2DH_REG_INT1_SRC, &reg_val);
...
#ifdef DT_ST_LIS2DH_0_IRQ_GPIOS_CONTROLLER_1
			    ...
			    const struct sensor_value *val)
{
	...
	if (attr == SENSOR_ATTR_SLOPE_TH) {
		...
		status = lis2dh_reg_write_byte(dev, LIS2DH_REG_INT1_DUR,
					        val->val1);
	}
...
static void lis2dh_thread_cb(void *arg)
{
	...
	if (atomic_test_and_clear_bit(&lis2dh->trig_flags,
		...
		/* clear interrupt 2 to de-assert int1 line */
		status = lis2dh_reg_read_byte(dev, LIS2DH_REG_INT1_SRC,
					      &reg_val);
		if (status < 0) {
...
int lis2dh_init_interrupt(struct device *dev)
{
	...
	status = lis2dh_burst_write(dev, LIS2DH_REG_INT1_THS, raw, sizeof(raw));
	if (status < 0) {
	...
	return lis2dh_reg_write_byte(dev, LIS2DH_REG_CTRL3,
				     LIS2DH_EN_INT1_INT1);
}
