...
static float32_value_t calibration_coefficient[MAX_INSTANCE_COUNT];
...
static int reset_min_max_measured_values_cb(uint16_t obj_inst_id, uint8_t *args,
					    uint16_t args_len)
{
	int i;
...
static struct lwm2m_engine_obj_inst *current_sensor_create(uint16_t obj_inst_id)
{
	...
	calibration_coefficient[index].val1 = 1;
	calibration_coefficient[index].val2 = 0;
	app_type[index][0] = '\0';
