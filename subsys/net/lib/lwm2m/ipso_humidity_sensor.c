...
#define LOG_MODULE_NAME ipso_humidity_sensor
...
static struct lwm2m_engine_obj humidity_sensor;
...
static struct lwm2m_engine_obj_inst *humidity_sensor_create(u16_t obj_inst_id)
{
	int index, i = 0;
...
static int ipso_humidity_sensor_init(struct device *dev)
{
	...
	humidity_sensor.obj_id = IPSO_OBJECT_ID;
	humidity_sensor.fields = fields;
	humidity_sensor.field_count = ARRAY_SIZE(fields);
	humidity_sensor.max_instance_count = MAX_INSTANCE_COUNT;
	humidity_sensor.create_cb = humidity_sensor_create;
	lwm2m_register_obj(&humidity_sensor);
