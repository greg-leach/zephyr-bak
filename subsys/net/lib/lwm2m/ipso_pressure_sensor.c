...
#define LOG_MODULE_NAME ipso_pressure_sensor
...
#define IPSO_OBJECT_ID      PRESSURE_OBJECT_RID
...
static struct lwm2m_engine_obj pressure_sensor;
...
static struct lwm2m_engine_obj_inst *pressure_sensor_create(u16_t obj_inst_id)
{
	int index, i = 0;
...
static int ipso_pressure_sensor_init(struct device *dev)
{
	...
	pressure_sensor.obj_id = IPSO_OBJECT_ID;
	pressure_sensor.fields = fields;
	pressure_sensor.field_count = ARRAY_SIZE(fields);
	pressure_sensor.max_instance_count = MAX_INSTANCE_COUNT;
	pressure_sensor.create_cb = pressure_sensor_create;
	lwm2m_register_obj(&pressure_sensor);
